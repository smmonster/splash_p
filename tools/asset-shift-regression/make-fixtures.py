...
def main():
    ...
    def add(name, expect, note, png):
        ...
        tmp.unlink()
        frame.unlink()
        cases.append({"name": name, "expect": expect, "note": note,
                      "frame": f"{name}.gray"})
    ...
    if args.video:
        ...
        cases.append({"name": "real", "expect": "shifted",
                      "note": "실제 반입 소재 — 카피라이트 10px 밀림",
                      "frame": "real.gray"})

    # 정상 — 통과해야 함
    add("ok-clean", "clean", "스틸컷 그대로 인코딩", still)
    ...
    add("gap-missing", "known-gap", "카피라이트 문구 삭제 — 평행이동이 아니라 검출 불가",
        shift_band(still, band, erase=True))
