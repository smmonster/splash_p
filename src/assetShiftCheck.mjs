 ...
 * 그중 큰 차이는 색차 검사가 담당한다. 작은 요소의 누락·추가와 좁은 영역만 1px 밀린 경우는
 * 두 검사 모두의 사각지대라, 겹쳐보기·깜빡임 육안 비교와 윤곽선 검사의 '확인 권장 지점'
 * 안내(edgeDiffCheck.mjs)가 이 구멍을 메운다.
 */
...
export const ASSET_SHIFT_PARAMS = {
  ...
  MIN_SHIFT: 0,       // 이 값을 넘는 이동량만 어긋남 후보.
                      //   1px 밀림도 실제 결함이라 0으로 둔다. 2로 두면 전체 대각선 1px 밀림
                      //   소재를 놓친다(실측: MIN_SHIFT=0 에서 95/95 블록 합의로 검출).
                      //   0으로 낮춰도 정상 소재 오탐은 ERR_RATIO 가 막는다(실측 오탐 0).
  ERR_RATIO: 0.6,     // 이동 후 오차가 제자리 오차의 이 비율 미만으로 줄어야 확정
                      //   실측 여유: 불일치 소재 0.16~0.18 vs 정상 소재 최악 0.85.
                      //   좁은 영역만 1px 밀린 소재는 0.70 으로 이 임계를 넘지 못해 놓친다.
                      //   임계를 0.75 로 올리면 잡히지만 정상 소재 0.85 와 여유가 없어 두지 않았다.
                      //   해당 케이스는 겹쳐보기·깜빡임 육안 확인이 담당한다.
